...
import { localTimezone, DAY_INITIAL, WEEK_ORDER } from '@/lib/schedule'
import { X, CalendarClock, Clock, Bell } from 'lucide-react'
import Button from '@/components/ui/Button'

interface Props {
  userId: string
  color: string
  defaultDay?: number          // día preseleccionado (0-6)
...
export default function AddScheduleBlockModal({ userId, color, defaultDay, onAdd, onClose }: Props) {
  const [title,       setTitle]       = useState('')
  const [description, setDescription] = useState('')
  const [days,        setDays]        = useState<number[]>(defaultDay != null ? [defaultDay] : [])
  const [startTime,   setStartTime]   = useState('08:00')
  const [endOn,       setEndOn]       = useState(false)
  const [endTime,     setEndTime]     = useState('09:00')
  const [blockColor,  setBlockColor]  = useState(PALETTE[0])
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{
            ...
            <CalendarClock size={16} color={color} />
          </div>
          <div>
            <h2 style={{ fontSize: '15px', fontWeight: 500, margin: 0 }}>Nuevo bloque de horario</h2>
            <p style={{ fontSize: '11px', color: 'var(--muted)', margin: '2px 0 0' }}>
              Se repite cada semana en los días elegidos
            </p>
          </div>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        <div>
          ...
          <input
            ...
            placeholder="Ej: Tomar medicina · Gimnasio · Reunión"
            autoFocus
